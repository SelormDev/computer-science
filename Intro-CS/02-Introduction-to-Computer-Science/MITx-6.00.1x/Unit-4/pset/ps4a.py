...
def getWordScore(word, n):
    ...
    letter_points_sum = 0
    for letter in word:
        letter_points_sum += SCRABBLE_LETTER_VALUES[letter]
        n -= 1
    word_score = letter_points_sum * len(word)
    ...
    while calculateHandlen(hand):
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                points_value = getWordScore(user_input, n)
                total_score = int(total_score) + int(points_value)
                print('"' + user_input + '"' + ' earned ' + str(points_value) + ' points.', 'Total : ' + str(total_score) + '\n')
                # Update the hand
